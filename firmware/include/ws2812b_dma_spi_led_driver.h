...
void WS2812BDMAInit( )
{
	...
	// Configure SPI
	// Choose SPI BR bits so that (SYSCLK / (2^ (BR+1))) ≈ 3MHz
#if FUNCONF_SYSTEM_CORE_CLOCK == 48000000
  #define WS2812_SPI_BR_BITS (3<<3) // 48/16 = 3MHz
#elif FUNCONF_SYSTEM_CORE_CLOCK == 24000000
  #define WS2812_SPI_BR_BITS (2<<3) // 24/8 = 3MHz
#else
  // Fallback: compute approximate BR at compile time for common clocks
  #if FUNCONF_SYSTEM_CORE_CLOCK >= 48000000
    #define WS2812_SPI_BR_BITS (3<<3)
  #elif FUNCONF_SYSTEM_CORE_CLOCK >= 24000000
    #define WS2812_SPI_BR_BITS (2<<3)
  #else
    #define WS2812_SPI_BR_BITS (1<<3) // 4x divider (may be too fast)
  #endif
#endif
	SPI1->CTLR1 =
